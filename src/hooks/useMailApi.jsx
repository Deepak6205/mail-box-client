...
export function useMailFolder(folder) {
  ...
  const fetchMails = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `https://mail-box-client-59016-default-rtdb.firebaseio.com/mails/${cleanEmail}/${folder}.json`
      );
  ...
  const deleteMail = useCallback(
    async (id) => {
      ...
      const cleanEmail = userEmail.replace(/\./g, "_");
      const url = `https://mail-box-client-59016-default-rtdb.firebaseio.com/mails/${cleanEmail}/${folder}/${id}.json`;
